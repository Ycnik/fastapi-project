...
from soldat.config import keycloak_admin_config
...
class UserService:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            roles_soldat = [role for role in roles if role["name"] == "soldat"]
            # Rolle 'soldat' in Keycloak fuer KeycloakAdmin.assign_client_role() und
            # ist ein dict mit Schluesseln "id", "name", "description", ...
            self.rolle_soldat = roles_soldat[0]
            logger.debug("rolle_soldat={}", self.rolle_soldat)
        except KeycloakConnectionError:
            logger.error("Keine Verbindung zu Keycloak! Ist Keycloak gestartet?")
            self.client_uuid = "N/A"
            self.rolle_soldat = None
    ...
    def create_user(self, user: User) -> str:
        ...
        self.keycloak_admin.assign_client_role(
            user_id=user_id, client_id=self.client_uuid, roles=[self.rolle_soldat]
        )
        return user_id
