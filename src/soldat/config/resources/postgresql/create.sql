...
CREATE TYPE rang AS ENUM ('REKRUT', 'SOLDAT', 'ELITE-SOLDAT', 'CAPTAIN', 'KOMMANDANT');
CREATE TYPE schweregrad AS ENUM ('LEICHT', 'MITTEL', 'SCHWER', 'KRITISCH');
CREATE TYPE waffe AS ENUM ('ODM_GEAR', 'Schrotflinte', 'Klingen');
...
CREATE TABLE IF NOT EXISTS verletzungen (
    id          INTEGER GENERATED ALWAYS AS IDENTITY(START WITH 1000) PRIMARY KEY,
    verletzungsbezeichnung    TEXT NOT NULL CHECK,
...
CREATE INDEX IF NOT EXISTS verletzungen_soldat_id_idx ON verletzungen(soldat_id);
