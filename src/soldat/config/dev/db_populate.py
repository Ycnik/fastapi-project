...
from soldat.config.config import resources_path
...
from soldat.config.dev_modus import dev_db_populate
from soldat.repository import engine
...
class DbPopulateService:
    ...
    def _load_csv_files(self) -> None:
        logger.debug("begin")
        tabellen: Final = ["soldat", "ausruestung", "verletzung"]
        csv_path: Final = "/init/soldat/csv"
        # siehe extras/compose/postgres/compose.init.yml
        with self.engine_admin.connect() as connection:
            connection.execute(text("SET search_path TO soldat;"))
